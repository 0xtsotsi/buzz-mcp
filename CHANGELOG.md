...
array (one entry per relay) and a `BUZZ_RELAY_URLS` JSON array is now
the canonical way to configure multiple relays. Phase 4's subscription
multiplexing will build on the pool's URL and auth plumbing.

### Added

- **`src/relay/pool.ts` — `RelayPool`** with:
  - `signedFetchEach(opts)`: fan-out across configured relays via
    `Promise.all`. Each call gets its own 5s timeout and AbortController.
  - 401 NIP-98 replay detection: 1.5 s sleep + 1 retry per relay.
  - NIP-11 probe via `nip11Info(relay, force?)` with a per-relay cache.
  - Channel cache (UUID resolution, 5-min TTL by default).
  - Per-relay `Host` header override via `BUZZ_RELAY_HOST_<n>`.
  - Per-call `allowFanout: false` to write only to the default relay.
  - Per-call `relays: [...]` to override the configured relay list.
  - Status bucketing via `outcomeFromStatus(...)` (re-uses Phase 2's
    `StatsStore`).
- **`src/tools/pool-write.ts` — `poolWrite(pool, event, opts)`**:
  - Single chokepoint for every write tool. Builds the event once, calls
    `gateWrite` (Phase 1), then fans out via the pool.
  - Returns `{ mcpBody, result, isError }`. The `mcpBody` JSON
    preserves the legacy fields via `responseExtras` (channel, target,
    emoji, etc.) and adds `posts: [...]` for fan-out visibility.
  - Throws an Error on the first non-2xx status, matching v0.1.x
    behavior. A 2xx with no event id is treated as accepted and the
    raw body is exposed as `raw`.
- **Test coverage:** 8 new tests in `test/unit/relay-pool.spec.ts`. All
  existing tests updated to pass a `makePool()` helper. 194 tests pass
  (was 186).

### Changed

- `createServer()` now constructs a single `RelayPool` and threads it
  through every write tool. The pool owns the relay list, NIP-11 probe,
  and channel cache.
- The five write tools that use the gate (`post_message`, `edit_message`,
  `react`, `create_channel`, `add_member`) and the four that don't
  (`create_job`, `approve_workflow`, `upload_media`, `post_thread_summary`)
  all go through `poolWrite`. The `signedFetchWithTimeout` helper is
  retained for read tools; the pool's `single` method uses the
  same NIP-98 header builder via `buildAuthHeader`.
- `mcp__buzz__buzz_post_message` (and the other write tools) now
  fan-out by default. Set `allowFanout: false` for single-relay writes.
  Per-call `relays: [...]` overrides the pool default.

### Test summary

`Test Files  22 passed (22)`
`Tests  194 passed (194)` (8 new in Phase 3)

Bumps version to 0.1.5.

### Known limitations

- File/audio/video uploads (`buzz_upload_media`) still use the
  multi-endpoint fallback; the pool does not yet fan-out the S3 upload.
  Phase 4 will consider this.
- The NIP-11 cache has no TTL; entries live for the lifetime of the
  server. Re-probe via `nip11Info(relay, true)` if the operator knows
  the relay's NIP-11 has changed.
